...
class CanManageTemperatureCheckAssignments(BasePermission):
    """
    Allows read-only access to any authenticated user for temperature check assignments.
    Allows write access (create, update, delete) only to managers.
    Object-level permissions ensure managers can only modify assignments
    within their own department (if they are assigned to one).
    """
    ...
    def has_permission(self, request, view):
        ...
        if request.method in SAFE_METHODS: # GET, HEAD, OPTIONS (for list and retrieve)
            return True # Further filtering by department done in get_queryset
    ...
    def has_object_permission(self, request, view, obj):
        """
        Object-level permission.
        - Read is fine for anyone authenticated (if they passed has_permission and get_queryset allows it).
        - Write (PUT, PATCH, DELETE) only for managers.
          - If manager has a department, they can only modify assignments in their department.
          - If manager has no department (but is a manager), they can modify any assignment.
        """
        if not request.user or not request.user.is_authenticated:
            return False

        # obj is an instance of TemperatureCheckAssignment, which has a 'department' field.

        if request.method in SAFE_METHODS:
            return True 

        # Write permissions: must be a manager
        try:
            if not request.user.profile.role == 'manager':
                return False # Not a manager, cannot write

            # Manager is trying to write.
            # If manager is assigned to a department, they can only modify objects in their department.
            if request.user.profile.department:
                return obj.department == request.user.profile.department
            else:
                # Manager is not assigned to a department but is a manager.
                # They can modify any assignment (acts like a global manager for this resource).
                return True 
        except AttributeError: # No profile
            return False

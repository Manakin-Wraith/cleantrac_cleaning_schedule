"""
Recipe Version Integration Module

This module provides functionality to integrate recipe versioning with production tasks.
It includes:
1. Middleware to automatically set recipe versions on production tasks
2. Utilities for scaling recipes based on production quantities
"""
...
from .recipe_models import RecipeVersion, RecipeProductionTask
...
class RecipeScalingService:
    ...
    def scale_recipe(recipe_version, target_quantity):
        ...
        """
        original_quantity = recipe_version.recipe.yield_quantity
        scaling_factor = Decimal(target_quantity) / Decimal(original_quantity)
        
        scaled_ingredients = {}
        for ingredient in recipe_version.recipe.ingredients.all():
            ...
            scaled_ingredients[ingredient.ingredient_name] = {
                'id': ingredient.id,
                'quantity': scaled_qty,
    ...
    def calculate_ingredient_requirements(production_task):
        ...
        target_quantity = production_task.scheduled_quantity
        
        # Get the specific version if stored, otherwise use the latest
...
# Signal handler to automatically set recipe_version if not provided
@receiver(pre_save, sender=RecipeProductionTask)
def set_recipe_version(sender, instance, **kwargs):
    """
    Signal handler to automatically set the recipe_version field on RecipeProductionTask
    if it's not already set.
    """
    if not instance.recipe_version and instance.recipe:
        latest_version = RecipeVersion.objects.filter(
            recipe=instance.recipe
        ).order_by('-version_number').first()
        
        if latest_version:
            instance.recipe_version = latest_version

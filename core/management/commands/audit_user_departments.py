...
Usage: python manage.py audit_user_departments
"""
...
class Command(BaseCommand):
    help = 'Audit user and department assignments between original and tenant databases'
    ...
    def get_original_users(self):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Error accessing original database: {e}"))
            return []
    ...
    def get_tenant_users(self):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Error accessing tenant database: {e}"))
            return []

    def handle(self, *args, **options):
        """Main audit function"""
        self.stdout.write("🔍 Starting comprehensive user/department audit...")
        self.stdout.write("=" * 80)
        
        # Get data from both databases
        self.stdout.write("📊 Fetching original database users...")
        original_users = self.get_original_users()
        self.stdout.write(f"   Found {len(original_users)} users in original DB")
        
        self.stdout.write("📊 Fetching tenant database users...")
        tenant_users = self.get_tenant_users()
        self.stdout.write(f"   Found {len(tenant_users)} users in tenant DB")
        ...
        self.stdout.write("🔍 ANALYSIS 1: Users missing UserProfile in tenant")
        ...
        if missing_profiles:
            self.stdout.write(self.style.ERROR(f"Found {len(missing_profiles)} users without UserProfile:"))
            for user in missing_profiles:
                self.stdout.write(f"   • {user['username']} (orig dept: {user['original_dept']}, role: {user['original_role']})")
        else:
            self.stdout.write(self.style.SUCCESS("All users have UserProfile records"))
        ...
        if missing_departments:
            self.stdout.write(self.style.ERROR(f"Found {len(missing_departments)} users without department:"))
            for user in missing_departments:
                self.stdout.write(f"   • {user['username']} (ID: {user['user_id']}, Profile: {user['profile_id']}, orig dept: {user['original_dept']})")
        else:
            self.stdout.write(self.style.SUCCESS("All users with profiles have department assignments"))
        ...
        if mismatches:
            self.stdout.write(self.style.ERROR(f"Found {len(mismatches)} department mismatches:"))
            for user in mismatches:
                self.stdout.write(f"   • {user['username']}: {user['original_dept']} → {user['tenant_dept']}")
        else:
            self.stdout.write(self.style.SUCCESS("No department mismatches found"))
        ...
        try:
            ...
            for dept in departments:
                ...
                self.stdout.write(f"   • {dept.name} (ID: {dept.id}) - {user_count} users")
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Error fetching departments: {e}"))
        
        # Summary
        self.stdout.write(f"\n" + "=" * 80)
        self.stdout.write("📋 SUMMARY")
        self.stdout.write("-" * 50)
        self.stdout.write(f"• Users without UserProfile: {len(missing_profiles)}")
        self.stdout.write(f"• Users without department: {len(missing_departments)}")
        self.stdout.write(f"• Department mismatches: {len(mismatches)}")
        
        if missing_departments:
            self.stdout.write(f"\n🔧 USERS CAUSING FRONTEND WARNINGS:")
            self.stdout.write("These are the 18 users being filtered out in the console:")
            for user in missing_departments:
                self.stdout.write(f"   • {user['username']} (ID: {user['user_id']})")
        
        self.stdout.write(f"\n" + self.style.SUCCESS("Audit complete!"))
